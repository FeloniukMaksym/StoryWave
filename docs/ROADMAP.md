...
- [x] При відкритті книги — підвантажуємо повний список файлів папки і кладемо в `playlist`
- [x] Сортування файлів за `name` (lexical)
- [x] На `ended` — `position = 0`, переходимо на наступний файл, автограємо (тут autoplay дозволено бо є user gesture в межах сесії)
- [x] Previous/Next кнопки активні, працюють
- [x] UI: список файлів книги на `PlayerPage`, тап на файл → перехід
- [x] Якщо в `playback_positions` є запис для файлу — стартує з нього, інакше з 0
- [x] Якщо це останній файл і він закінчився — pause + повідомлення "Book finished"
